...
for folder in os.listdir(ROOT):
    ...
    # Chemins des fichiers PET et SEG
    ...
    if not (os.path.exists(pet_file) and os.path.exists(seg_file)):
        print(f"[SKIP] Paire PET/SEG incomplète dans {folder}")
        continue
    ...
    # Copie PET → images/
    shutil.copy(pet_file, os.path.join(IMAGES_DIR, new_name))

    # Copie SEG → labels/
    shutil.copy(seg_file, os.path.join(LABELS_DIR, new_name))

    print(f"[OK] Copied PET+SEG for {folder}")
